...
class Agent(BaseModel):
    ...
    def build_cnn_net(self,config):
        ...
        with tf.variable_scope('CNN-ZFNet'):
            ...
            # CNN_output
            self.cnn_output, self.w['output_w'], self.w['output_b'] = fc_layer(self.cnn_l5_flat, 4096, activation = activation, w_initializer = w_initializer, b_initializer = b_initializer, name = 'output')
    ...
    def build_dqn_net(self,config):
        ...
        with tf.variable_scope('target'):
            # DQN_target_fc1
            self.dqn_t_l1, self.dqn_t_w['l1_w'], self.dqn_t_b['l1_b'] = fc_layer(inp, 1024, activation = activation, w_initializer = w_initializer, b_initializer = b_initializer, name = 'dqn_t_l1')

            # DQN_target_fc2
            self.dqn_t_l2, self.dqn_t_w['l2_w'], self.dqn_t_b['l2_b'] = fc_layer(self.dqn_t_l1, 1024, activation = activation, w_initializer = w_initializer, b_initializer = b_initializer, name = 'dqn_t_l2')

            # DQN_target_output
            self.dqn_t_l3, self.dqn_t_w['output_w'], self.dqn_t_b['output_b'] = fc_layer(self.dqn_t_output, config.action_size, w_initializer = w_initializer, b_initializer = b_initializer, name = 'dqn_t_l3')

        # update weights
        with tf.variable_scope('pred_to_target'):
            self.dqn_t_w_inp = {}
            self.dqn_t_w_assign_op = {}

            for key in self.dqn_w.keys():
                self.dqn_t_w_inp[key] = tf.placeholder('float32', self.dqn_t_w[key].get_shape().as_list(), name = key)
                self.dqn_t_w_assign_op[key] = self.dqn_t_w[key].assign(self.dqn_t_w_inp[key])
